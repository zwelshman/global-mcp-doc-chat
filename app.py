...
def extract_version_from_description(description):
    """Try to pull a version string from an MCP tool description."""
    if not description:
        return None
    patterns = [
...
async def probe_servers(mcp_config):
    """
    Connect to each server, list tools, and attempt to read a version
    from the tool descriptions or by calling a search/read tool with a
    version-focused query.
    Returns dict: {name: {"ok": bool, "tools": int, "version": str|None, "error": str|None}}
    """
    results = {}
    async with AsyncExitStack() as stack:
        for name, url in mcp_config.items():
            try:
                ...
                # Try to extract version from any tool description
                version = None
                for t in tools:
                ...
                # If no version found in descriptions, try calling a search tool
                if not version:
                    search_tool = next(
...
# ---------------------------------------------------------------------------
# Connection status panel - probes servers and shows version info
# ---------------------------------------------------------------------------
...
with st.expander("Connected servers", expanded=True):
    ...
    for name in mcp_config:
        ...
        label = name
        if lib_meta:
            label = name + " (" + lib_meta["desc"] + ")"
        status_str = "Connected" if ok else "Failed"
