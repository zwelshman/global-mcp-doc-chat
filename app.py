...
if mcp_config:
    st.success(str(len(mcp_config)) + " server(s) active")
...
py_active     = [l["shortname"] for l in LIBRARY_CATALOGUE if l["lang"] == "Python" and l["shortname"] in mcp_config]
r_active      = [l["shortname"] for l in LIBRARY_CATALOGUE if l["lang"] == "R"      and l["shortname"] in mcp_config]
custom_active = [n for n in mcp_config if n not in catalogue_names]

parts = []
if py_active:
parts.append("Python: " + ", ".join(py_active))
if r_active:
parts.append("R: " + ", ".join(r_active))
if custom_active:
parts.append("Custom: " + ", ".join(custom_active))
st.info("  \n".join(parts))
